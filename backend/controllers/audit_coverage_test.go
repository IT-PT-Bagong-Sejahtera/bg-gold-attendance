...
func TestEveryMutationRouteHasAnAuditDisposition(t *testing.T) {
	audited := map[string]string{
		...
		"PATCH /employees/{employeeID}/kiosk-pin":         "employee.kiosk_pin.reset",
		...
		"POST /kiosk-devices":                             "kiosk.activate",
		"DELETE /kiosk-devices/{kioskID}":                 "kiosk.revoke",
		"POST /kiosk/attendance/actions":                  "attendance.action",
	...
	exempt := map[string]string{
		...
		"POST /kiosk/attachments/attendance-selfie":    "staged kiosk media is finalized and referenced by the audited attendance action",
		"POST /kiosk/employee-status":                  "credential-gated status lookup is read-only despite using a POST body",
